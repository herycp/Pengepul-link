...
def extract_links_from_page(html, base_url):
    ...
    if not articles:
        # Fallback: cari semua link yang menuju ke /drama/ atau /movie/
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if href.startswith("/") and ("drama" in href or "movie" in href or "film" in href):
    ...
    for article in articles:
        # Cari judul dan link
        title_elem = article.select_one("h2 a, h3 a, h4 a, .entry-title a, a[rel='bookmark']")
        if not title_elem:
            continue
